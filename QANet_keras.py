...
from BatchSlice import BatchSlice
...
def QANet(config, word_mat=None, char_mat=None, cove_model=None):
    ...
    # embedding char
    CharEmbedding = Embedding(char_input_size, char_dim, weights=[char_mat], input_length=char_limit, name='char_embedding')
    ...
    highway_layers = [Conv1D(filters, 1, kernel_initializer=init, kernel_regularizer=regularizer)]
    for i in range(2):
        highway_layers.append(Conv1D(filters, 1, kernel_initializer=init,
                                     kernel_regularizer=regularizer, activation='sigmoid'))
        highway_layers.append(Conv1D(filters, 1, kernel_initializer=init,
    ...
    for i in range(4):
        DepthwiseConv_share_1.append([DepthwiseConv2D((7, 1), activation='relu',
                                                      kernel_initializer=init,
                                                      kernel_regularizer=regularizer,
                                      ...
                                      Conv2D(filters, 1,
                                             ...
                                             kernel_regularizer=regularizer)])
    # shared attention
    SelfAttention_share_1 = [Conv1D(2 * filters, 1,
                                    kernel_initializer=init,
                                    kernel_regularizer=regularizer),
                             Conv1D(filters, 1,
                                    kernel_initializer=init,
    ...
    # shared feed-forward
    FeedForward_share_1 = []
    FeedForward_share_1.append(Conv1D(filters, 1,
                                      kernel_initializer=init,
                                      kernel_regularizer=regularizer,
                                      activation='relu'))
    FeedForward_share_1.append(Conv1D(filters, 1,
    ...
    x_cont = attention_block(SelfAttention_share_1, x_cont, c_mask, dropout)
    ...
    x_ques = attention_block(SelfAttention_share_1, x_ques, q_mask, dropout)
    ...
    for i in range(7):
        ...
        for i in range(2):
            DepthwiseConv_share_2_temp.append([DepthwiseConv2D((5, 1),
                                                               activation='relu',
                                                               kernel_initializer=init,
                                                               kernel_regularizer=regularizer,
                                                               padding='same',
        ...
        SelfAttention_share_2.append([Conv1D(2 * filters, 1,
                                             kernel_initializer=init,
                                             kernel_regularizer=regularizer),
                                      Conv1D(filters, 1,
                                             kernel_initializer=init,
                                             kernel_regularizer=regularizer),
                                      MultiHeadAttention(filters, num_head, dropout=0.1, bias=False)])
        FeedForward_share_2.append([Conv1D(filters, 1,
                                           kernel_initializer=init,
                                           kernel_regularizer=regularizer,
                                           activation='relu'),
                                    Conv1D(filters, 1,
    ...
    for i in range(3):
        ...
        for j in range(7):
            ...
            x = attention_block(SelfAttention_share_2[j], x, c_mask, dropout, l=j, L=7)
